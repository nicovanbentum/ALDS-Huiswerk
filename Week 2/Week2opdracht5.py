...
def qsort_worst_pivot(a, low=0,high=-1):
    ...
    if high == -1:
        high = len(a) -1
    if low < high:
        swap(a, low, low) #Basically has no effect but instead of choosing a random index as pivot, it just uses the lowest
        m = low
        for j in range(low+1, high+1):
            if a[j] < a[low]:
                cmp_count = cmp_count + 1
...
qsort_worst_pivot(test_list)
print("number of comparisons (worst pivot): " + str(cmp_count))
#################################################
